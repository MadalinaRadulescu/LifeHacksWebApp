...
public class UserDataController : Controller
{
    ...
    public UserDataController(DatabaseContext db)
    {
    ...
    [HttpGet("{id}")]
    public UserData GetUserDataByDefaultId(string id) => _userData.Get(id);

    [HttpPost("add")]
    public async Task AddUserData([FromBody] UserData userData) => await _userData.Add(userData);
    
    [HttpDelete("remove/{id}")]
    public async Task RemoveUserData(string id) => await _userData.Remove(id);
